    ...
    """
        ...
        if input_reprs is None:
            ...
            with tf.variable_scope((scope if untied_embeddings else "electra") + "/embeddings", reuse=tf.AUTO_REUSE):
                ...
                self.all_embeddings = embedding_postprocessor(
        ...
        with tf.variable_scope(scope, default_name="electra"):
            if self.all_embeddings.shape[-1] != bert_config.hidden_size:
                self.embedding_output = tf.layers.dense(
                    self.all_embeddings, bert_config.hidden_size, name="embeddings_project"
                )
    ...
    for layer_idx in range(num_hidden_layers):
        with tf.variable_scope("layer_%d" % layer_idx):
            with tf.variable_scope("attention"):
                ...
                all_selves.append(attention_output)
